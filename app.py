...
def mark_attendance():
    ...
    if request.method == 'POST':
        for student in students:
            ...
            # Fixed: Attendance model doesn't have date/status fields
            att = Attendance(student_id=student.id, month=date.today().strftime('%B'),
                           present_days=1 if status == 'Present' else 0, total_days=1)
            db.session.add(att)
            if status == 'Absent':
...
def admin_student_page_control():
    ...
    if request.method == 'POST':
        ...
        new_user = User(username=roll_no, role='student')
...
def admin_exam(student_id):
    ...
    if request.method == 'POST':
        ...
        exam = Exam(student_id=student_id, term=term, subject=subject, marks_obtained=float(marks))
        db.session.add(exam)
        db.session.commit()
